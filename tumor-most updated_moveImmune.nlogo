...
to setup-env ; this is for initial setup of the envs
  (ifelse
  ...
  ask immunes [
    ...
    ; else
    [
      ask patches [set pcolor green];setup-patches ; if the chosen environment is dynamic
    ]
   )
end

to setup-tumors
  create-tumors 1 [setxy pxcor / 2 pycor / 2 ] ; place tumor cells in the center
  ask tumors [
...
to tumor-chemokine
  ...
  [
    if count turtles > 30
    [set chemokine chemokine + 2]
  ]

  diffuse chemokine 1 ;diffuse to neighboring patch
  ask patches
  [ set chemokine chemokine * 0.9 ; evaporate
    set pcolor scale-color green chemokine 0.1 3]
  tick
end

to imm-chemokine ; immune cell migrating towards a chemokine
    ask immunes
  [ if chemokine > 0
    [move-to-chemokine]
    wiggle
   fd 1 ;                     1 > specificit ;ignore unless there is enough
  ]
  ;tick
end

;to turn-toward-chemokine ;turn in direction of greatest chemokine
;  let ahead [chemokine] of patch-ahead 1
;    let myright [chemokine] of patch-right-and-ahead sniff-angle 1
;  let myleft [chemokine] of patch-left-and-ahead sniff-angle 1
...
to move-to-chemokine
  let chemo-ahead chemo-angle   0
  let chemo-right chemo-angle  45
  let chemo-left  chemo-angle -45
  if (chemo-right > chemo-ahead) or (chemo-left > chemo-ahead)
  [ ifelse chemo-right > chemo-left
    [ rt 45 ]
    [ lt 45 ] ]
end
...
to imm-kill
  ...
  ifelse tumor-in-sight = nobody [
      fd 1
      set tumor-in-sight min-one-of ( tumors in-radius 5 with [
      not any? my-links ] ) [distance myself]
      if tumor-in-sight != nobody [
      create-link-with tumor-in-sight
    ]
    ]
    [   face tumor-in-sight
    ifelse distance tumor-in-sight > 1 [
      fd 1
    ] [
      move-to tumor-in-sight
        ask tumor-in-sight [die]
      set tumor-in-sight nobody
    ]
    ]
  ]
